    ...
    <div class="vote-container">
      ...
      <div class="vote-body">
        ...
        <template v-if="poll.algorithm === 'weighted'">
          <div class="weighted-sum">{{ usedWeight }} / 100</div>
          <el-progress
            :percentage="usedWeight"
          ...
          <div class="vote-options weighted-list">
            <div v-for="opt in poll.options" :key="opt.id" class="weighted-item">
              ...
              <div class="weighted-control">
                <el-button
                  size="small" circle
                  :disabled="(weightValues[opt.id] || 0) <= 0"
                  @click="adjustWeight(opt.id, -5)"
                >-</el-button>
                <el-input-number
                  v-model="weightValues[opt.id]"
                  :min="0"
                  :max="100"
                  :step="5"
                  size="small"
                ...
                <el-button
                  size="small" circle
                  :disabled="(weightValues[opt.id] || 0) >= 100"
                  @click="adjustWeight(opt.id, 5)"
                >+</el-button>
        ...
        <template v-if="poll.algorithm === 'borda'">
          <div class="vote-hint">拖拽选项排序（最上为最喜欢，暂用点击操作）</div>
          <div class="vote-options borda-list">
            <div
              v-for="(opt, idx) in bordaOrdered"
              :key="opt.id"
              class="borda-item"
            >
              ...
              <div class="borda-actions">
                <el-button
                  ...
                  circle
                  :disabled="idx === 0"
                  @click="moveBorda(idx, -1)"
                />
                <el-button
                  size="small" :icon="Bottom"
                  circle
                  :disabled="idx === bordaOrdered.length - 1"
                  @click="moveBorda(idx, 1)"
                />
              </div>
            </div>
        ...
        <!-- 提交按钮 -->
        <div v-if="poll.status === 'active'" class="submit-area">
          <el-button
            type="primary"
            size="large"
            :disabled="!canSubmit"
          ...
          <el-button size="large" @click="$router.push(`/results/${poll.id}`)">
            查看实时结果
          </el-button>
        </div>
...
const usedWeight = computed(() =>
  Object.values(weightValues).reduce((s, v) => s + (v || 0), 0)
)
...
const canSubmit = computed(() => {
  ...
  switch (poll.value.algorithm) {
    ...
    case 'scoring': return Object.values(scoreValues).every(v => v && v > 0)
...
onMounted(() => {
  if (poll.value) {
    ...
    // 初始化权重
    poll.value.options.forEach(o => { weightValues[o.id] = 0 })
    // 初始化打分
    poll.value.options.forEach(o => { scoreValues[o.id] = 0 })
  }
})
</script>
